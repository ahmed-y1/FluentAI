...
describe("EyeContactTracker rolling %", () => {
  it("returns 0 for empty history",              () => { const t=new T(); expect(t.pct()).toBe(0); });
  ...
  it("resets to 0 after reset()",               () => { const t=new T(); t.setHistory(Array(100).fill(true)); t.reset(); expect(t.pct()).toBe(0); });
});